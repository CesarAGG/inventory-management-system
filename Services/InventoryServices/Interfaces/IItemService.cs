...
{
    Task<ServiceResult<(string Id, string Boundaries, int NewSequenceValue)>> RegenerateIdAsync(string inventoryId, RegenerateIdRequest request, ClaimsPrincipal user);
    Task<ServiceResult<object>> ValidateCustomIdAsync(string inventoryId, ValidateIdRequest request, ClaimsPrincipal user);
